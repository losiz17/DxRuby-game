module Stage3
  class Director
    def initialize
      ...
      @yoko=Image.load('images/yoko.png')
      @yoko.set_color_key([0,0,0])
      @damage=Image.load('images/damage.png')
      @damage.set_color_key([255,0,0])
...
@number[9]=Image.load('images/number_9.png')
      ...
      @font=Font.new(16)
      @background=Image.load('images/stage3.png')
    ...
    def construction
      ...
      @space = CP::Space.new
      @space.gravity = CP::Vec2.new(0, 400)
      @speed = 1 / 60.0
      ...
      [
        ...
        [Window.width/2-150, Window.height/4+150, 100, 50,@yoko],
        [Window.width/2-250, Window.height/4+150, 100, 50,@yoko],
        [Window.width/2    , Window.height/4+150, 100, 50,@yoko],
        [Window.width/2+100, Window.height/4+150, 100, 50,@yoko],
...
###################################ゲーム画面の変遷
      ...
      if  @ball.body.p.y.to_i> 672 && @ball.body.p.y.to_i<768 && @ball.body.p.x.to_i> 976 && @ball.body.p.x.to_i<1024
        @bgm.stop
        Scene.set_current_scene(:clear)
      end

      if  @ball.body.p.y.to_i> 350 && @ball.body.p.y.to_i<440 && @ball.body.p.x.to_i> 50 && @ball.body.p.x.to_i<230
        $life-=1
        @bgm.stop
        Scene.set_current_scene(:stage3)
        if $life==0
      ...
      when  1
        ...
        if @block_num<9
          ...
          unless x>=0&&x<=100&&y>=0&&y<=100
            unless  y> 350-32 && y<440 && x> 50-128 && x<230
...
if @block_num<9
        if Input.mouse_push?(M_LBUTTON)
        unless x>=0&&x<=100&&y>=0&&y<=100
          unless  y> 350-64 && y<440 && x> 50-64 && x<230
